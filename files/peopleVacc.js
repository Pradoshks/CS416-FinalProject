function peopleVacc(svg, usAllData, countries, countryName, data, data_rel_pop_vaccine) {
  ...
  let text1 =`Vaccines take years to be developed. But thanks to the latest technology and knowledge gained from previous outbreaks,
  on Dec 2 2020, 1st vaccine was approved in UK.
   Vaccines seem to help curb the cases. `

  text2='As of today, 1,084,820,502 (1.08B) people all over the world have been fully vaccinated which is 13.9% of the world population.'
  ...
  switchG.append('rect')
    ...
    .attr('width', 20)
    .attr('fill', '#238b45')
    .on('mouseover', function (event) { d3.select(this).attr('fill', 'red')})
    .on('mouseout', function (event) { d3.select(this).attr('fill', '#238b45')})
